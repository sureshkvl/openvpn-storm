...
(function() {
  ...
  Openvpn = (function() {
    ...
    function Openvpn(settings) {
      ...
      fs.mkdir("" + this.config, (function(_this) {
        return function(result) {
          return fs.mkdir("/var/stormflash/plugins/openvpn", function(result) {
            _this.servers = new Servers("/var/stormflash/plugins/openvpn/servers.db");
            return _this.users = new Users("/var/stormflash/plugins/openvpn/users.db");
          });
        };
      })(this));
    }
    ...
    Openvpn.prototype.addserver = function(server, callback) {
      ...
      return this.generateConfig(configData, (function(_this) {
        return function(configFile) {
          var data, env, out, serverInfo;
          ...
          return _this.settings.agent.start(_this.serverInstance.id, function(key, pid) {
            ...
            if (key instanceof Error) {
              return callback(new Error("Failed to start openvpn server instance. Error is " + key));
            }
            _this.settings.agent.log("pid result ", pid);
            configData.pid = pid;
            _this.settings.agent.log("instance result ", _this.serverInstance.id);
            configData.instanceId = _this.serverInstance.id;
            _this.settings.agent.log("server id result ", configData.id);
            result = _this.servers.add(configData.id, configData);
            _this.settings.agent.log("printing result for openvpn server db add: ", result);
            ccdpath = result.data["client-config-dir"];
            if (ccdpath != null) {
              try {
                fs.mkdir("" + ccdpath, function() {});
                _this.settings.agent.log('created ccd path');
              } catch (_error) {
                err = _error;
                _this.settings.agent.log('Error : ', err);
              }
            }
    ...
    Openvpn.prototype.generateConfig = function(configData, callback) {
      var filename, gconfig, i, key, server, service, val, _i, _len;
      ...
      filename = this.config + "/" + configData.id + ".conf";
    ...
    Openvpn.prototype.adduser = function(serverid, user, callback) {
      var ccdpath, configData, err, file, filename, gconfig, i, key, res, result, service, val, _i, _len;
      try {
      ...
      if (!res) {
        return callback(new Error("Error: Unknown Server instance"));
      }
      ccdpath = res.data["client-config-dir"];
      filename = ccdpath + "/" + ("" + file);
      this.settings.agent.log("Debug: adduser() - file is " + file + ", ccdpath is " + ccdpath + " and res is ", res);
      service = "openvpn";
      gconfig = '';
      for (key in user) {
        val = user[key];
    ...
    Openvpn.prototype.deleteuser = function(serverid, userid, callback) {
      var ccdpath, cname, email, entry, exists, file, filename, path, res, ulist, user, _i, _len;
      path = require('path');
      res = this.servers.get(serverid);
      ulist = this.users.list();
      if (ulist) {
        for (_i = 0, _len = ulist.length; _i < _len; _i++) {
          entry = ulist[_i];
          this.log("Debug: checking for ", entry.data);
          if (entry && entry.data && entry.data.cname === userid) {
            user = entry;
          }
        }
      }
      ...
      if (!exists) {
        ...
        this.settings.agent.log('file removed already');
