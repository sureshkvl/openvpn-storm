...
(function() {
  ...
  Openvpn = (function() {
      ...
      return this.generateConfig(server, (function(_this) {
        return function(configFile) {
          ...
          return _this.settings.agent.start(_this.serverInstance.id, function(key, pid) {
            ...
            server.pid = pid;
            server.instanceId = _this.serverInstance.id;
            configData = new ServerData(_this.serverid, server);
            result = _this.servers.add(configData.id, configData);
            ccdpath = result.data["client-config-dir"];
            if (ccdpath != null) {
              try {
                fs.mkdir("" + _this.config.datadir, function() {});
    ...
    Openvpn.prototype.generateConfig = function(server, callback) {
      var filename, gconfig, i, key, service, val, _i, _len;
      service = "openvpn";
      gconfig = '';
      for (key in server) {
        val = server[key];
      ...
      filename = this.config + "/" + this.serverid + ".conf";
