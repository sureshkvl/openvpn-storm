(function() {
  ...
  Openvpn = (function() {
    ...
    function Openvpn(settings) {
      ...
      fs.mkdir("/var/stormflash/plugins/openvpn", function() {});
    ...
    Openvpn.prototype.addserver = function(server, callback) {
      var _this = this;
      return this.generateConfig(server, function(configFile) {
        var data, serverInfo;
        serverInfo = {
          "name": "openvpn",
          "path": "/usr/sbin",
          ...
          case "object":
            if (val instanceof Array) {
              for (_i = 0, _len = val.length; _i < _len; _i++) {
                i = val[_i];
                if (key === "route") config += "" + key + " " + i + "\n";
                if (key === "push") config += "" + key + " \"" + i + "\"\n";
              }
            }
            break;
          case "number":
    ...
    Openvpn.prototype.adduser = function(serverid, user, callback) {
      ...
      if (res == null) callback(new Error("Error: Unknown Server instance"));
      ...
      this.settings.agent.restart(this.serverInstance.id, function(key, pid) {
