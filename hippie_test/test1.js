...
hippie().json().base(baseurl).post('/openvpn/server').send(serverdata).expectStatus(200).end(function(err, res, body) {
  var id;
  if (err) {
    throw err;
  }
  console.log(body);
  id = body.id;
  return hippie().json().base(baseurl).get('/openvpn/server/' + id).expectStatus(200).end(function(err, res, body) {
    if (err) {
      throw err;
    }
    console.log("id   ", id);
